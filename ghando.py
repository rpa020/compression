...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.pretrained:
        ...
        prune(model, torch.zeros(1, 3, 224, 224), model.__class__.__name__)
...
def prune(model, dummy_inputs, model_name):
    
    original_size = pruning.utils.count_params(model)
    model.cpu().eval()
    ...
    params = pruning.utils.count_params(model)
    print("  Params: %s => %s" % (original_size, params))
